...
(function() {
  var clicky, goToProfile, initialize;

  clicky = function(number) {
    console.log(number);
    $.ajax({
  ...
  initialize = function(user_id) {
    var address, bounds, boundsListener, geocoder, getURL, i, infoWindow, infoWindowContent, map, mapOptions, marker, markers, me, position, u, users;
    geocoder = void 0;
      ...
      geocoder.geocode({
        ...
        if (status === google.maps.GeocoderStatus.OK) {
          if (status !== google.maps.GeocoderStatus.ZERO_RESULTS) {
            ...
            postUrl = '/users/' + user_id + '/edit_latlong';
            dt = {
    ...
    while (u < users.length) {
      ...
      $.ajax({
        ...
        success: function(user_json) {
          ...
          uj = user_json['person'];
          num = users[u];
          infoWindowContent.push(['<div class="info_content">' + '<h5>' + uj['name'] + '</h5>' + '<a href ="users/' + num + '/show"' + '><img src="' + uj['prof_pic'] + '" style="border-radius: 20px 20px 20px 20px" height="150px"></a>' + '<p>' + uj['home'] + '</p>' + '<a style="text-decoration: none" href="/users/' + num + '/dishes"> View Dishes </a>' + '</div>']);
